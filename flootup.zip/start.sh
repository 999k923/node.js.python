...
# =========================================================
# 获取端口（平台提供 SERVER_PORT/PORT）
# =========================================================
[ -n "${SERVER_PORT:-}" ] && PORTS_STRING="$SERVER_PORT" || PORTS_STRING="${PORT:-}"
if [ -z "${PORTS_STRING:-}" ]; then
  echo "[警告] 未找到 SERVER_PORT/PORT，使用默认端口 8080"
  PORTS_STRING="8080"
fi
read -ra AVAILABLE_PORTS <<< "$PORTS_STRING"
PORT_COUNT=${#AVAILABLE_PORTS[@]}
[ $PORT_COUNT -eq 0 ] && echo "[错误] 未找到端口（需要平台提供 SERVER_PORT/PORT）" && exit 1
